...
**Module path:** `github.com/darinkelkhoff/connectedCli` (binary `conctl` built from `./cmd/conctl`). Adjust the GitHub owner if different.
...
Run:
```bash
cd /Users/darinkelkhoff/git/personal/connectedCli
go mod init github.com/darinkelkhoff/connectedCli
...
builds:
  - id: conctl
    ...
    goarch: [amd64, arm64]
    ldflags:
      - -s -w -X github.com/darinkelkhoff/connectedCli/internal/cli.Version={{.Version}}
archives:
  - formats: [tar.gz]
brews:
  - name: conctl
...
1. Create the `darinkelkhoff/homebrew-tap` GitHub repo (once).
...
in the tap. End users then run `brew install darinkelkhoff/tap/conctl`.
```
